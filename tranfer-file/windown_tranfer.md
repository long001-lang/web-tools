...
```c
anhlong@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```c
anhlong@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```c
anhlong@htb[/htb]$ sudo pip3 install pyftpdlib
...
```c
anhlong@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```c
anhlong@htb[/htb]$ echo  | base64 -d > hosts
```

```c
anhlong@htb[/htb]$ md5sum hosts 
...
```c
anhlong@htb[/htb]$ pip3 install uploadserver
...
```c
anhlong@htb[/htb]$ python3 -m uploadserver
...
```c
anhlong@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
...
```c
anhlong@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```c
anhlong@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```c
anhlong@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
